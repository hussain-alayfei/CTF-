...
- **Dynamic score changes never rewrite old solve rows.** Day 9 uses difficulty
  tiers (Easy 100 · Medium 250 · Hard 400 · Danger 500) with solve-order decay;
  Danger follows `525 → 500 → 475 → 450`. `solves.points_awarded` is an immutable
  receipt of what that player earned; existing days default to no decay.
