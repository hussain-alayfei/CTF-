...
- **Live event day:** **Day 4 "Securing Networks"** is the active authored day.
  ...
  is authored (see below) but kept locked until go-live; Days 6–10 are placeholders.
- **Day 4 = 7 core + 2 extra, ALL `is_dynamic` (per-player flags).** Core =
  ...
  `target-box/` is gone.)
- **Day 5 = Privacy, per-player dynamic.** Multi-stage, tool-forced, AI-resistant
  set (cookies/history, metadata de-anon, Tor exit-node + onion layers, zero-width
  stego, k-anonymity linkage). Artifacts in `public/challenges/day5/`, generated by
  `scripts/gen-day5-artifacts.py`. Answer key in `ADMIN_MANUAL_DAY5.md`.
...
public/               static assets served at web root
  challenges/         downloadable challenge artifacts (day4/*, day5/*,
                      vault.png, hidden.png) + s3cr3t-vault.html, day5 hosted
                      pages, robots.txt
