...
**Day 5 model (v4):** medium/hard/danger are **live hands-on browser tasks**, NOT the old uniform file-XOR. Only **2 files**: `places.sqlite` + `metadata-mirage.jpg`. Fingerprint answers = ciphertext XOR SHA-256(real-env). **Danger (Re-Identified v2):** anonymized data removed from bundle entirely — lives only inside `query_anon_db` Postgres RPC body (VALUES clause). Page shows query interface only; students must make real API calls (rate-limited, 40-query budget) to discover the unique k-anon failure. `src/lib/reidentData.ts` deleted. `src/lib/dayfive.ts` = crypto helpers for fingerprint challenges.
