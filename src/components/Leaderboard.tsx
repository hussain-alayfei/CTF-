            ...
            return (
              <li
                ...
                className={`flex items-center gap-3 border-b border-terminal-border/50 px-4 py-2.5 ${
                  me ? 'bg-terminal-green/10' : i < 3 ? 'bg-white/[0.02]' : ''
                }`}
              >
                <span className="w-7 shrink-0 text-center text-sm font-bold text-terminal-dim">
                  {i < 3 ? medal[i] : i + 1}
                </span>
                <span className={`flex-1 truncate font-semibold ${me ? 'text-terminal-green' : 'text-terminal-green/90'}`}>
                  {r.username}
                  {me && <span className="ml-2 text-[10px] uppercase tracking-widest text-terminal-dim">you</span>}
                </span>
